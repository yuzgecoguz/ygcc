...
class Jbex extends BaseExchange {
  describe() {
    return {
      id: 'jbex',
      name: 'JBEX',
      ...
      urls: {
        api: 'https://www.jbex.com',
        ws: 'wss://www.jbex.com/openapi/quote/ws/v1',
        doc: 'https://apidocs.jbex.com',
      },
      timeframes: {},
      fees: {
  ...
  constructor(config = {}) {
    super(config);
    // JBEX: params go as query string, not JSON body
    this._wsClients = new Map();
    this._wsHandlers = new Map();
  }

  // ---------------------------------------------------------------------------
  // Authentication — HMAC-SHA256, Binance-compatible
  // ---------------------------------------------------------------------------

  /**
   * JBEX uses Binance-compatible signing:
  ...
  _toJbexSymbol(symbol) {
    // 'BTC/USDT' -> 'BTCUSDT'
    return symbol.replace('/', '');
  }

  _fromJbexSymbol(jbexSymbol) {
    // 'BTCUSDT' -> 'BTC/USDT' (via marketsById lookup)
    if (this.marketsById && this.marketsById[jbexSymbol]) {
      return this.marketsById[jbexSymbol].symbol;
    }
    const quotes = ['USDT', 'USDC', 'BTC', 'ETH', 'BNB', 'BUSD', 'TRY', 'EUR'];
    for (const q of quotes) {
      if (jbexSymbol.endsWith(q) && jbexSymbol.length > q.length) {
        return jbexSymbol.slice(0, -q.length) + '/' + q;
      }
    }
    return jbexSymbol;
  }
  ...
  _unwrapResponse(data) {
    if (data && typeof data === 'object' && !Array.isArray(data)) {
      ...
      if (code && parseInt(code, 10) < 0) {
        ...
        this._handleJbexError(code, msg);
      }
    }
    return data;
  }
  ...
  _handleJbexError(code, msg) {
    const full = this.id + ' ' + code + ': ' + msg;
    const numCode = parseInt(code, 10);

    // Binance-compatible error codes
  ...
  _parseTicker(data) {
    ...
    const symbol = sym ? this._fromJbexSymbol(sym) : undefined;
  ...
  _parseOrder(data, fallbackSymbol) {
    ...
    const jbexSym = safeString(data, 'symbol');
    const symbol = jbexSym ? this._fromJbexSymbol(jbexSym) : fallbackSymbol;
  ...
  async fetchTicker(symbol) {
    const jbexSymbol = this._toJbexSymbol(symbol);
    const data = await this._request('GET', '/openapi/quote/v1/ticker/24hr', { symbol: jbexSymbol }, false, 1);
    const result = this._unwrapResponse(data);

    const tickerData = Array.isArray(result) ? result.find(t => t.symbol === jbexSymbol) : result;
    if (!tickerData) {
  ...
  async fetchOrderBook(symbol, limit = undefined) {
    const jbexSymbol = this._toJbexSymbol(symbol);
    const params = { symbol: jbexSymbol };
  ...
  async fetchOpenOrders(symbol = undefined) {
    ...
    const params = {};
    if (symbol) {
      params.symbol = this._toJbexSymbol(symbol);
    }
  ...
  // ---------------------------------------------------------------------------
  // WebSocket — Binance-compatible depth subscription
  // ---------------------------------------------------------------------------
