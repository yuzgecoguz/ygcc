...
// ═══════════════════════════════════════════════════════════════
// 1. Module Exports — Jbex
// ═══════════════════════════════════════════════════════════════
describe('Module Exports — Jbex', () => {
  it('exports Jbex class', () => {
    assert.strictEqual(typeof lib.Jbex, 'function');
  });

  it('exports lowercase alias', () => {
    assert.strictEqual(lib.jbex, lib.Jbex);
  });

  it('includes jbex in exchanges list', () => {
    assert.ok(lib.exchanges.includes('jbex'));
  });
});
...
// 2. Jbex Constructor
// ═══════════════════════════════════════════════════════════════
describe('Jbex Constructor', () => {
  let exchange;
  beforeEach(() => { exchange = new Jbex(); });

  it('sets id to jbex', () => {
    assert.strictEqual(exchange.describe().id, 'jbex');
  });

  it('sets name to JBEX', () => {
    assert.strictEqual(exchange.describe().name, 'JBEX');
  });
...
// 4. Response Handling
// ═══════════════════════════════════════════════════════════════
describe('Jbex Response Handling', () => {
  let exchange;
  beforeEach(() => { exchange = new Jbex(); });
...
describe('Jbex Parsers', () => {
  ...
  it('_parseTicker extracts Binance-style fields', () => {
    ...
    assert.strictEqual(t.percentage, 1.69);
  });
...
// 6. Helper Methods
// ═══════════════════════════════════════════════════════════════
describe('Jbex Helper Methods', () => {
  let exchange;
  beforeEach(() => { exchange = new Jbex(); });

  it('_toJbexSymbol BTC/USDT -> BTCUSDT', () => {
    assert.strictEqual(exchange._toJbexSymbol('BTC/USDT'), 'BTCUSDT');
  });

  it('_toJbexSymbol ETH/BTC -> ETHBTC', () => {
    assert.strictEqual(exchange._toJbexSymbol('ETH/BTC'), 'ETHBTC');
  });

  it('_fromJbexSymbol resolves via marketsById', () => {
    exchange.marketsById = { 'BTCUSDT': { symbol: 'BTC/USDT' } };
    assert.strictEqual(exchange._fromJbexSymbol('BTCUSDT'), 'BTC/USDT');
  });

  it('_fromJbexSymbol fallback splits common quotes', () => {
    exchange.marketsById = {};
    assert.strictEqual(exchange._fromJbexSymbol('ETHUSDT'), 'ETH/USDT');
  });

  it('_getBaseUrl returns api URL', () => {
    assert.strictEqual(exchange._getBaseUrl(), 'https://www.jbex.com');
  });
});
...
// 7. Error Mapping
// ═══════════════════════════════════════════════════════════════
describe('Jbex Error Mapping', () => {
  let exchange;
  beforeEach(() => { exchange = new Jbex(); });

  it('-1002 -> AuthenticationError', () => {
    assert.throws(() => exchange._handleJbexError('-1002', 'Unauthorized'), AuthenticationError);
  });

  it('-1003 -> RateLimitExceeded', () => {
    assert.throws(() => exchange._handleJbexError('-1003', 'Too many requests'), RateLimitExceeded);
  });

  it('-1013 -> InvalidOrder', () => {
    assert.throws(() => exchange._handleJbexError('-1013', 'Invalid quantity'), InvalidOrder);
  });

  it('-2011 -> OrderNotFound', () => {
    assert.throws(() => exchange._handleJbexError('-2011', 'Unknown order'), OrderNotFound);
  });

  it('-1121 -> BadSymbol', () => {
    assert.throws(() => exchange._handleJbexError('-1121', 'Invalid symbol'), BadSymbol);
  });

  it('-1001 -> ExchangeNotAvailable', () => {
    assert.throws(() => exchange._handleJbexError('-1001', 'Internal error'), ExchangeNotAvailable);
  });

  it('unknown code -> ExchangeError', () => {
    assert.throws(() => exchange._handleJbexError('-9999', 'Unknown'), ExchangeError);
  });
});
...
// ═══════════════════════════════════════════════════════════════
describe('Jbex HTTP Error Handling', () => {
  let exchange;
  beforeEach(() => { exchange = new Jbex(); });
...
// 10. Mocked API Calls
// ═══════════════════════════════════════════════════════════════
describe('Jbex Mocked API Calls', () => {
  let exchange;
  beforeEach(() => {
    exchange = new Jbex({ apiKey: 'testkey', secret: 'testsecret' });
...
// ═══════════════════════════════════════════════════════════════
// 12. JBEX vs Others Differences
// ═══════════════════════════════════════════════════════════════
describe('JBEX vs Others Differences', () => {
  let exchange;
  beforeEach(() => { exchange = new Jbex({ apiKey: 'k', secret: 's' }); });

  it('Binance-compatible: X-BH-APIKEY header + signature param', () => {
    const result = exchange._sign('/openapi/v1/account', 'GET', {});
  ...
  it('concatenated symbol format BTCUSDT', () => {
    assert.strictEqual(exchange._toJbexSymbol('BTC/USDT'), 'BTCUSDT');
  });
  ...
  it('Binance error codes (-1002, -1003, etc.)', () => {
    assert.throws(() => exchange._handleJbexError('-1002', 'Auth'), AuthenticationError);
    assert.throws(() => exchange._handleJbexError('-1003', 'Rate'), RateLimitExceeded);
    assert.throws(() => exchange._handleJbexError('-1121', 'Symbol'), BadSymbol);
  });
});
...
// ═══════════════════════════════════════════════════════════════
describe('Crypto — hmacSHA256 for JBEX', () => {
...
// ═══════════════════════════════════════════════════════════════
// 14. WebSocket — Binance-compatible depth
// ═══════════════════════════════════════════════════════════════
describe('WebSocket — Binance-compatible depth', () => {
  let exchange;
  beforeEach(() => { exchange = new Jbex(); });

  it('WS URL includes jbex', () => {
    assert.ok(exchange.describe().urls.ws.includes('jbex'));
  });
